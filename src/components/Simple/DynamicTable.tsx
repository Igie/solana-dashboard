            ...
            >
              <tr>
                {columns.map((col, colIndex) => (
                  <th
                    key={colIndex}
                    className={`lg:px-2 px-1 py-0.5 border text-xs border-gray-700 bg-gray-800 lg:font-semibold font-normal text-nowrap ${col.className ?? ""}`}
                  >
                    {col.header}
                  </th>
                ))}
            ...
            {data.map((row, rowIndex) => (
              ...
              >
                {columns.map((col, colIndex) => (
                  <td
                    key={colIndex}
                    className={`lg:px-2 px-1 py-0.5 border border-gray-700 text-xs lg:text-md text-nowrap ${col.className ?? ""}`}
                  >
